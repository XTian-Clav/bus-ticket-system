...
<div class="bg-navy rounded-2xl p-10 text-white shadow-sm flex items-center justify-between flex-wrap gap-6">
    <div>
        <p class="text-gold font-semibold tracking-wide uppercase text-sm mb-2">Welcome back</p>
        <h2 class="text-3xl font-extrabold mb-2"><?= e($_SESSION['username'] ?? '') ?></h2>
    ...
    <a href="<?= url('/views/user/schedules.php') ?>"
       class="px-6 py-3 bg-gold text-navy font-semibold rounded-lg hover:bg-gold-light transition flex-shrink-0">
        Browse Schedules
    </a>
</div>
